...
\right)}
\end{equation}
where $\symNumReg\,\symSmallCorrectionFunc(\symCount_0/\symNumReg)$ and $\symNumReg\,\symLargeCorrectionFunc(1-\symCount_{\symRegRange+1}/\symNumReg)$ replace $\symCount_0$ and $\symCount_{\symRegRange+1}$ in the \emph{raw} estimate \eqref{equ:intermediate_estimate}, respectively. The functions $\symSmallCorrectionFunc$ and $\symLargeCorrectionFunc$ are defined as
\begin{equation}
...
\end{equation}
almost the same estimator as given in \eqref{equ:small_range_estimate}. Here we used that $\symPowerSeriesFunc$ is known to be approximately equal to 1.
