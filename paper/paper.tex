...
-\texttt{expm1}(-\symX_{\ast\symRegVal})$ and $\symY_{\ast\symRegVal} = 1 - \symZ_{\ast\symRegVal}$ and not as defined in \eqref{equ:definition_xyz}. In this way the numerical error of both, $\symY_{\ast\symRegVal}$ and $\symZ_{\ast\symRegVal}$, is minimized and only a single exponential function evaluation is needed.
...
\subsection{Results}
We used the popular Broyden-Fletcher-Goldfarb-Shanno (BFGS) algorithm to solve this optimization. We used the implementation provided by the GNU scientific library \cite{Galassi2016}.
