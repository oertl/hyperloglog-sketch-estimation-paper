...
\comm{.5}{alternatively, take $\symNumReg\cdot\symLargeCorrectionFunc(1 - \symCountVariate_{\symRegRange+1}/\symNumReg)$ from precalculated lookup table}
...
\comm{.5}{alternatively, take $\symNumReg\cdot\symSmallCorrectionFunc(\symCountVariate_0/\symNumReg)$ from precalculated lookup table}
\State \Return$\symAlpha_\infty\symNumReg^2 / \symZ$
\comm{.5}{$\symAlpha_\infty := 1/(2\log(2))$}
\EndFunction
\\
...
\caption{Standard deviations of different cardinality estimators for a HyperLogLog sketch with paramters $\symPrecision = 12$ and $\symRegRange=20$ as functions of the true cardinality.}
...
\Else
\State $\symX \gets \symNumReg'/b \cdot \log(1+\symB/\symA)$ 
...
\end{aligned}
\end{equation}
and the fact that HyperLogLog sketches can be easily merged using \eqref{equ:hll_union}. The last equation is also known as inclusion-exclusion principle and can be used to calculate the intersection size. Unfortunately, the estimation error does not scale well for inclusion-exclusion principle. For small intersection sizes the estimation error can get very large \cite{Dasgupta2015}. In the worst case, the estimate can be even negative without additional restriction. Therefore, it was proposed to combine HyperLogLog sketches with MinHash sketches \cite{Pascoe2013, Cohen2016}, which improves the estimation error, even though at the expense of significant more space consumption. 
...
\symZ_{\symSetASuffix\symRegVal}
\geq
\frac{\symZ_{\symSetXSuffix\symRegVal}}{2}
+
\frac{
\symZ_{\symSetXSuffix\symRegVal}+
\symY_{\symSetXSuffix\symRegVal}
...
\geq
\frac{
\max(\symZ_{\symSetXSuffix\symRegVal},
\symZ_{\symSetASuffix\symRegVal})
}{2}
=
\frac{1}{2}
\left(1-e^{-\frac{\max(\symPoissonRate_\symSetASuffix,\symPoissonRate_\symSetXSuffix)}{\symNumReg 2^{\symRegVal}}}\right)
\approx
\frac{\max(\symPoissonRate_\symSetASuffix,\symPoissonRate_\symSetXSuffix)}{\symNumReg 2^{\symRegVal+1}}
$.
The last approximation is valid, if $\max(\symPoissonRate_\symSetASuffix,\symPoissonRate_\symSetXSuffix)\ll\symNumReg 2^{\symRegVal+1}$, and shows that the argument of the logarithm is large enough to be accurately represented by double-precision floating-point numbers. Similar argumentation holds for all other logarithmic terms and also the divisions that appear in \eqref{equ:df_dphi}.
...
%TODO
%In all other cases the cardinality of $\symSetA$ and $\symSetB$ 
...
We used the popular Broyden-Fletcher-Goldfarb-Shanno (BFGS) algorithm \cite{Press2007} to solve this optimization problem. We used the implementation provided by the GNU scientific library \cite{Galassi2016}.
...
TODO see results \cref{tbl:joint_estimation_cases}, \cref{tbl:joint_estimation_a}, \cref{tbl:joint_estimation_b}, \cref{tbl:joint_estimation_x},
\cref{tbl:joint_estimation_abx}


\begin{table}
\centering
\caption{List of true cardinalities of the pair-wise disjoint sets $\symSetA$, $\symSetB$, and $\symSetX$, for which maximum likelihood estimation from two HyperLogLog sketches with parameters $\symPrecision=20$ and $\symRegRange=44$ representing sets $\symSetA\cup\symSetX$ and  $\symSetB\cup\symSetX$ was investigated. The average number of function \eqref{equ:max_likelihood_phi_func} and gradient evaluations \eqref{equ:df_dphi} are also given as needed by our implementation of \cref{alg:joint_cardinality_estimation} that uses the BFGS algorithm.
}
\label{tbl:joint_estimation_cases}
\csvreader[before reading=\tiny, tabular=r|rrr|rr|rr, table head= & 
\multicolumn{3}{c|}{true cardinalities} &
\multicolumn{1}{c}{Jaccard index} &
\multicolumn{1}{c|}{cardinality ratio} &
\multicolumn{2}{c}{avg number evaluations}
\\
\# &  
\multicolumn{1}{c}{$|\symSetA|$} & 
\multicolumn{1}{c}{$|\symSetB|$} & 
\multicolumn{1}{c|}{$|\symSetX|$} & 
\multicolumn{1}{c}{$\frac{|\symSetX|}{|\symSetA\cup\symSetB\cup\symSetX|}$} & 
\multicolumn{1}{c|}{$\frac{|\symSetA\cup\symSetX|}{|\symSetB\cup\symSetX|}$} &
\multicolumn{1}{c}{$\symFunc$} &
\multicolumn{1}{c}{$\nabla\symFunc$}
\\\hline]
...
avgNumGradientEvaluations=\avgNumGradientEvaluations,
}{
\thecsvrow & 
\num{\trueCardA} & 
\num{\trueCardB} & 
...
\numformattwo{\avgNumGradientEvaluations}
}
\end{table}

\begin{table}
\centering
\caption{The mean, the standard deviation, and the root-mean-square error of the relative estimation error are given for all the cases given in \cref{tbl:joint_estimation_cases} when estimating $|\symSetA|$ using the conventional approach and the maximum likelihood method, respectively. The root-mean-square error improvement factor of the maximum likelihood method is shown in the last column.}
\label{tbl:joint_estimation_a}
\csvreader[before reading=\tiny, tabular=r|rrr|rrr|r, table head=
 & 
...
\begin{table}
\centering
\caption{The mean, the standard deviation, and the root-mean-square error of the relative estimation error are given for all the cases given in \cref{tbl:joint_estimation_cases} when estimating $|\symSetB|$ using the conventional approach and the maximum likelihood method, respectively. The root-mean-square error improvement factor of the maximum likelihood method is shown in the last column.}
...
\begin{table}
\centering
\caption{The mean, the standard deviation, and the root-mean-square error of the relative estimation error are given for all the cases given in \cref{tbl:joint_estimation_cases} when estimating $|\symSetX|$ using the conventional approach and the maximum likelihood method, respectively. The root-mean-square error improvement factor of the maximum likelihood method is shown in the last column.}
...
\begin{table}
\centering
\caption{The mean, the standard deviation, and the root-mean-square error of the relative estimation error are given for all the cases given in \cref{tbl:joint_estimation_cases} when estimating $|\symSetA\cup\symSetB\cup\symSetX|$ using the conventional approach and the maximum likelihood method, respectively. The root-mean-square error improvement factor of the maximum likelihood method is shown in the last column.}
...
%We have presented new algorithms for the estimation of cardinalities from HyperLogLog sketches. All approaches are based on the Poisson approximation. As shown, an unbiased estimator for the rate in the Poisson model, is also unbiased for the estimation of the cardinality. We have observed that this approach also works well for Poisson rate estimators that are not perfectly unbiased. Therefore, it would be interesting to investigate which conditions on an approximately unbiased Poisson rate estimator are sufficient to guarantee approximate unbiasedness if used as cardinality estimator.

%For the estimation from a single sketch, we have developed two fast algorithms that make use of the Poisson approximation. The first uses the original estimator extended by theoretically motivated correction terms. The second is based on the maximum-likelihood method and solves the corresponding optimization problem using the secant method. Both algorithms are inherently unbiased and can be implemented without empirical determined data.

%Finally the maximum likelihood method was also applied to the estimation of set operation result sizes where the operands are represented by HyperLogLog sketches. The new approach significantly improves the cardinality estimates for set intersection, differences, and unions when compared to the inclusion-exclusion principle. An extension to more than two HyperLogLog sketches is possible. However, the effort scales like $\symBigO(2^n)$ if 
