...
\leq \symPowerSeriesFunc(\symX) \leq 1 + \symEpsPowerSeriesFunc$ with $\symEpsPowerSeriesFunc:=\num{9.885e-6}$ (see \cref{fig:power_series_func}). This value can also be found using Fourier analysis as shown in in \cref{app:analysis_xi}.
...
\begin{equation}
\symPowerSeriesFunc(\symX) = 1 + \Re\!\left(\sum_{\symIndexL=1}^{\infty}
\symA_\symIndexL \exp(2\pi\symIndexL\symX\symImaginary)\right)
\end{equation}
with coefficients
\begin{equation}
\symA_\symIndexL = 2\int_0^1 \symPowerSeriesFunc(\symX) \exp(-2\pi\symIndexL\symX\symImaginary)d\symX
=
2\log(2) \int_{-\infty}^{\infty} 2^\symX
e^{-2^\symX}\exp(-2\pi\symIndexL\symX\symImaginary)d\symX.
\end{equation}
...
\symY^{-\frac{2\pi\symIndexL\symImaginary}{\log(2)}}d\symY
= 
2\Gamma\!\left(1-\frac{2\pi\symIndexL\symImaginary}{\log(2)}\right)
\end{equation}
where $\Gamma$ denotes the gamma function. Using the identity $\left|\Gamma(1+\symImaginary\symX)\right| = 
...
\begin{equation}
\max(|\symPowerSeriesFunc(\symX)-1|) \leq \sum_{\symIndexL=1}^{\infty} 
|\symA_\symIndexL|
\leq \num{9.885e6}.
\end{equation}
