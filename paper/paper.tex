...
=
\frac{\symNumReg^2}{2\log(2)\left(\symCount_0^{\text{corr}} + \sum_{\symRegVal=1}^\symRegRange \symCount_\symRegVal 2^{-\symRegVal} + \symCount_{\symRegRange+1}^{\text{corr}}2^{-(\symRegRange+1)}\right)}
\end{equation}
where $\symCount_0^{\text{corr}}$ and $\symCount_{\symRegRange+1}^{\text{corr}}$ are corrections for $\symCount_0$ and $\symCount_{\symRegRange+1}$, respectively (compare \eqref{equ:intermediate_estimate}). They are given by
\begin{equation}
\symCount_0^{\text{corr}} := \symNumReg
\cdot
\left(
\frac{\symCount_0}{\symNumReg}
+
...
and
\begin{equation}
\symCount_{\symRegRange+1}^{\text{corr}}
:=
\symNumReg
\cdot
\left(
...
-
\left(
1-\frac{\symCount_{\symRegRange+1}}{\symNumReg}
\right)
\right).
\end{equation}
