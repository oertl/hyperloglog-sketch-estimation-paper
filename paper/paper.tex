...
{2\log(2)
\left(
\symSmallCorrectionFunc(\symCount_0,\symNumReg) + \sum_{\symRegVal=1}^\symRegRange \symCount_\symRegVal 2^{-\symRegVal} + \symLargeCorrectionFunc(\symCount_{\symRegRange+1},\symNumReg) 2^{-(\symRegRange+1)}
\right)}
\end{equation}
...
\begin{equation}
\symSmallCorrectionFunc(\symCount,\symNumReg) := \symNumReg
\cdot
\left(
\frac{\symCount}{\symNumReg}
+
\sum_{\symRegVal=1}^\infty
\left(\frac{\symCount}{\symNumReg}\right)^{2^\symRegVal} 2^{\symRegVal-1}
\right)
\end{equation}
and
\begin{equation}
\symLargeCorrectionFunc(\symCount,\symNumReg)
:=
\symNumReg
\cdot
\sum_{\symRegVal=1}^\infty
\left(1-\frac{\symCount_{\symRegRange+1}}{\symNumReg}\right)^{2^{-\symRegVal}}
\cdot
\left(1-\left(1-\frac{\symCount_{\symRegRange+1}}{\symNumReg}\right)^{2^{-\symRegVal}}\right)
\cdot
2^{-(\symRegVal-1)}
\end{equation}
...
see \algoref{alg:corrected_raw_estimation}
...
\State $\symZ\gets 0.5\cdot \symLargeCorrectionFunc(\symCount_{\symRegRange+1},\symNumReg)$
\For{$\symRegVal\gets \symRegRange, 1$}
\State $\symZ\gets0.5\cdot\left(\symZ + \symCount_\symRegVal\right)$
\EndFor
\State $\symZ\gets\symZ+\symSmallCorrectionFunc(\symCount_0,\symNumReg)$
\State \Return$\symNumReg^2 / \left(2\log(2)\symZ\right)$
\EndFunction
\\
\Function {$\symSmallCorrectionFunc$}{$\symCount,\symNumReg$}
\If{$\symCount=\symNumReg$}
\State\Return$\infty$
\EndIf
\State $\symX\gets\symCount/\symNumReg$
\State $\symY\gets1$
...
\Until{$\symZ=\symZ'$}
\State\Return$\symNumReg\cdot\symZ$
\EndFunction
\\
\Function {$\symLargeCorrectionFunc$}{$\symCount,\symNumReg$}
\State $\symX\gets(\symNumReg-\symCount)/\symNumReg$
\State $\symY\gets1$
\State $\symZ\gets0$
\Repeat
...
see \autoref{fig:corrected_raw_avg_exec_time}
