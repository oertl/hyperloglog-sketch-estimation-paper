...
\begin{algorithm}
\caption{Insertion of a data element $\symDataItem$ into a HyperLogLog data structure that consists of $\symNumReg=2^\symPrecision$ registers. All register values $\vec{\symRegVal} = (\symRegVal_1,\ldots,\symRegVal_\symNumReg$) are initially equal to 0 and remain in the range $[0, \symRegRange+1]$.}
\label{alg:insert}
\begin{algorithmic}
...
Under the assumption of a strong universal hash function, the probability mass function for the register values $\vec{\symRegVal}=(\symRegVal_1,\ldots,\symRegVal_\symNumReg)$ of a HyperLogLog sketch with parameters $\symPrecision$ and $\symRegRange$ is given by
\begin{equation}
