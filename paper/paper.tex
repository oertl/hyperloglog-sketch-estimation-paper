...
\Comment $\symStopEpsilon = 10^{-2}$ (see \cref{sec:comp_ml_estimate})
\While{$\Delta\symX > \symX\cdot\symStopDelta$} \Comment secant method iteration
...
Quasi-Newton methods are commonly used for finding the maximum of such multi-dimensional functions. They require the calculation of the first derivatives which are given by
...
The calculation of \eqref{equ:max_likelihood_phi_func} and its derivatives requires some care when calculating $\symY_{\ast\symRegVal}$ and $\symZ_{\ast\symRegVal}$. Since $\symX_{\ast\symRegVal}$ is nonnegative, the value range of both is $[0,1]$. In order to reduce the numerical error of $\symZ_{\ast\symRegVal}$ for small $\symX_{\ast\symRegVal}$, it is essential to use the function $\texttt{expm1}(\symX):=e^\symX-1$ that is available in most programming languages. If $\symX_{\ast\symRegVal}$ is smaller than $\log(2)$, we calculate $\symY_{\ast\symRegVal}$ and $\symZ_{\ast\symRegVal}$ via $\symZ_{\ast\symRegVal} = 
-\texttt{expm1}(-\symX_{\ast\symRegVal})$ and $\symY_{\ast\symRegVal} = 1 - \symZ_{\ast\symRegVal}$ and not as defined in \eqref{equ:definition_xyz}. In this way the numerical error of both, $\symY_{\ast\symRegVal}$ and $\symZ_{\ast\symRegVal}$, is minimized and only a single exponential function needs to be evaluated.

Apart from that, the numerical evaluation of \eqref{equ:max_likelihood_phi_func} is straightforward. The arguments of all logarithms are in the range $[0,1]$. The case that some argument is equal to zero which would cause the logarithm to be negative infinite, does not occur in practice. Consider for example the logarithm evaluation associated with $\symCountVariate^{<}_{1\symRegVal}$ which is only relevant if $\symCountVariate^{<}_{1\symRegVal} > 0$. In this case however, we can be certain that the cardinality of $\symSetA\cup\symSetX$ is at least 1. Therefore, it is expected that at least one of the two maximum likelihood estimates $\symPoissonRateEstimate_\symSetASuffix$ and $\symPoissonRateEstimate_\symSetXSuffix$ is in the order of 1. If the optimization algorithm starts with appropriate initial values, function evaluations for which $\min(\symPoissonRate_\symSetASuffix,\symPoissonRate_\symSetXSuffix)\ll 1$ are not expected.
Furthermore, the argument of the logarithm can be bounded by $\symZ_{\symSetXSuffix\symRegVal}
+
\symY_{\symSetXSuffix\symRegVal}
\symZ_{\symSetASuffix\symRegVal}
\geq
\min(\symZ_{\symSetXSuffix\symRegVal},\symZ_{\symSetASuffix\symRegVal})
=
1-e^{-\frac{\min(\symPoissonRate_\symSetASuffix,\symPoissonRate_\symSetXSuffix)}{\symNumReg 2^\symRegVal}}
\approx
\frac{\min(\symPoissonRate_\symSetASuffix,\symPoissonRate_\symSetXSuffix)}{\symNumReg 2^\symRegVal}
$.
The last approximation is valid if $\min(\symPoissonRate_\symSetASuffix,\symPoissonRate_\symSetXSuffix)\ll\symNumReg 2^\symRegVal$ and shows that the argument of the logarithm is large enough to be accurately represented by the double-precision floating-point format. Similar argumentation holds for all other logarithmic terms and also the divisions that appear in \eqref{equ:df_dphia},\eqref{equ:df_dphib}, and \eqref{equ:df_dphix}.
...
\State \textbf{return} $(\symPoissonRateEstimate_{\symSetASuffix\symSetXSuffix},\symPoissonRateEstimate_{\symSetBSuffix\symSetXSuffix},0)$
\EndIf
...
\State $\symPhi_\symSetASuffix = \log(\max(1, \symPoissonRateEstimate_{\symSetASuffix\symSetBSuffix\symSetXSuffix} - \symPoissonRateEstimate_{\symSetBSuffix\symSetXSuffix})/\symNumReg)$
\State $\symPhi_\symSetBSuffix = \log(\max(1, \symPoissonRateEstimate_{\symSetASuffix\symSetBSuffix\symSetXSuffix} - \symPoissonRateEstimate_{\symSetASuffix\symSetXSuffix})/\symNumReg)$
...
\subsection{Results}
We used the popular Broyden-Fletcher-Goldfarb-Shanno (BFGS) algorithm to solve this optimization problem. We used the implementation provided by the GNU scientific library \cite{Galassi2016}.
