...
(1-\symAlpha_\infty).
\end{equation}
This finding makes the HyperLogLog algorithm an interesting candidate for locality-sensitive hashing. What is also promising is the much simpler preprocessing because it only involves a single hash function (compare \cref{alg:insert}). In contrast, min-wise hashing is very costly, because many hash functions evaluations are necessary for all the needed permutations \cite{Li2011}.
