...
\label{alg:compress}
\begin{algorithmic}
\Function {Compress}{\symDataItem, $\boldsymbol{\symRegValVariate}$}
...
The new estimation algorithm is very elegant, because it does neither contain magic numbers nor special cases as the original algorithm.
...
%\centering
%\caption{TODO}
%\label{tbl:joint_estimation_jaccard}
%\csvreader[before reading=\scriptsize, tabular=r|rrr|rrr|r, table head=
% & 
%\multicolumn{3}{c|}{conventional approach}
%&
...
%\\\hline]
%{joint_cardinality_calculation.csv}{
%inclExclMeanJaccardIdx=\inclExclMeanJaccardIdx,
%inclExclStdDevJaccardIdx=\inclExclStdDevJaccardIdx,
%inclExclRMSEJaccardIdx=\inclExclRMSEJaccardIdx,
%maxLikeMeanJaccardIdx=\maxLikeMeanJaccardIdx,
%maxLikeStdDevJaccardIdx=\maxLikeStdDevJaccardIdx,
%maxLikeRMSEJaccardIdx=\maxLikeRMSEJaccardIdx,
%improvementRmseJaccard=\improvementRmseJaccard
%}{
%\thecsvrow & 
%\numformat{\inclExclMeanJaccardIdx} & 
%\numformat{\inclExclStdDevJaccardIdx} & 
%\numformat{\inclExclRMSEJaccardIdx} & 
%\numformat{\maxLikeMeanJaccardIdx} & 
%\numformat{\maxLikeStdDevJaccardIdx} & 
%\numformat{\maxLikeRMSEJaccardIdx} &
%\numformattwo{\improvementRmseJaccard}
%}
%\end{table}
...
\begin{figure}
\centering
\includesvg[width=0.6\textwidth]{equal-register-probability}
...
\symAlpha_\infty$). This means that the HyperLogLog algorithm is not as memory-efficient as 1-bit minwise hashing, but significantly better than 32-bit minwise hashing. 

It is worth mentioning that estimating the Jaccard distance by just counting equal registers does not use all the information contained by the HyperLogLog sketch. The method described in \cref{sec:cardinality_estimation_set_intersections} would allow a more accurate estimation of the Jaccard distance by using the estimates for intersection and union sizes. This could be used for additional more precise filtering when searching for similar items.

Preprocessing is very costly for minwise hashing, because many hash function evaluations are necessary for all the needed permutations \cite{Li2011}. In contrast, the HyperLogLog algorithm requires only a single hash function evaluation which reduces preprocessing costs. In addition, HyperLogLog sketches can be easily merged, which is not possible for $\symNumBitsMinwiseHashing$-bit minwise hash signatures.
...
\end{equation}
Consequently, $|\symA_{\symIndexL}| \leq 
|\symA_{2}|
\left(\sqrt{\frac{
3
}{
2 e^\symB
}}\right)^{\symIndexL-2}$ for $\symIndexL\geq 2$ and 
\begin{equation}
...
\num{9.154e-12}.
\end{equation}
For the maximum deviation from 1 we finally have
\begin{equation}
\num{9.884e-6}
\leq
|\symA_1|
