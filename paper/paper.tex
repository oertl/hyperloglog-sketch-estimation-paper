...
For large cardinalities in the order of $2^{\symPrecision+\symRegRange}$ for which a lot of registers already are already in a saturated state meaning that they have already reached the maximum possible value $\symRegRange + 1$, the \emph{raw} estimate underestimates the cardinalities. For the 32-bit hash value case $(\symPrecision+\symRegRange=32)$ which was considered in \cite{Flajolet2007}, following correction formula was proposed to take these saturated registers into account
\begin{equation}
...
The entire original estimation algorithm as presented in \cite{Flajolet2007} including small and large range corrections is summarized by \algoref{alg:estimate_original}. 
\begin{algorithm}
...
The large range correction is not satisfying either. The large range correction does not reduce the estimation error. It even increases the estimation error. However, instead of underestimating the cardinalities, they are now overestimated. A simple approach to avoid the large range correction at all is the use of larger hash values $(\symPrecision+\symRegRange>32)$ which shift the biased region of $\symCardinalityOriginalEstimate$ to higher cardinalities. However, in case $\symRegRange \geq 30$ 6-bit registers as used in \cite{Heule2013} are required to allow representing all values in range $[0, \symRegRange+1]$. If 64-bit hash values are used as proposed in \cite{Heule2013} $(\symPrecision+\symRegRange=64)$, a correction for large cardinalities is not required. Cardinalities in the order of $2^{64}$ for which saturated register become a problem never occur in practice.
...
In the following we describe a new cardinality estimation method that does not require special handling for small and large cardinalities and which also does not require any empirically determined constants or corrections. The method is based on the maximum likelihood method. 

The maximum likelihood method was already applied to HyperLogLog sketches in \cite{Clifford2012}. However, there, a different element insertion algorithm was assumed that works with time complexity $\symBigO(\symNumReg)$, because each register calculates its own hash value from the inserted element and updates its current value accordingly. In comparison, \algoref{alg:insert} uses only a single hash value that defines which register needs to be updated. As a consequence, the register values are dependent, which makes the joint probability distribution of all register values more complex.

In the following we use the Poisson approximation in order to simplify the model. Despite this approximation the maximum likelihood estimate will give us high quality estimates for the entire cardinality range as simulation results will show later. Furthermore, we will demonstrate that consequent application of the maximum likelihood method to the Poisson model directly reveals that the cardinality estimate needs to be roughly proportional to the harmonic mean for intermediate cardinality values. The history of the HyperLogLog algorithm shows that the \emph{raw} estimate \eqref{alg:estimate_original} was first found after several attempts using the geometric mean \cite{Durand2003, Flajolet2007}.
...
\end{equation}
if, the function $\symHelper(\symX)$ is defined as
\begin{equation}
\label{equ:helper}
...
It is easy to see that the $\symPoissonRateEstimate$ remains equal or is larger after updating the HyperLogLog data structure as defined by \algoref{alg:insert}. An update potentially changes the cardinality vector $(\symCount_0,\ldots,\symCount_{\symRegRange+1})$ to $(\symCount_0,\ldots,\symCount_\symIndexI-1,\ldots,\symCount_\symIndexJ+1,\ldots,\symCount_{\symRegRange+1})$ where $\symIndexI < \symIndexJ$. Writing \eqref{equ:func} as 
\begin{multline}
\symFunc(\symX)
:=
\symCount_0 \symX
...
\item The function $\symHelper(\symX)$ only needs to be evaluated at values $\lbrace\frac{\symX}{2^{\symRegVal'_\text{max}}},\frac{\symX}{2^{\symRegVal'_\text{max}-1}}\ldots,\frac{\symX}{2^{\symRegVal_\text{min}}}\rbrace$ where $\symRegVal'_\text{max} := \min(\symRegVal_\text{max}, \symRegRange)$. This series corresponds to geometric series with ratio 2. A straightforward calculation using \eqref{equ:helper} is very expensive because of the exponential function. However, if we know $\symHelper\left(\frac{\symX}{2^{\symRegVal'_\text{max}}}\right)$ all other required function values can be easily obtained using the identity
\begin{equation}
